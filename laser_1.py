...
class EstRobot:
	...
	def update_imu(self,yaw_rate, yaw_acc, dt):
		...
		# 更新速度和位置
		self.est_vel = np.clip(self.est_vel + real.acc_x * dt, -MAX_SPEED, MAX_SPEED)
	...
	def update_laser(self,laser_obstacle):
		laser_x = []
		laser_y = []
		data = []
		up_width = []
		...
		laser_est_yaw = fsolve(func, x0=estrobot.est_yaw)[0]  # x0 是浮点初始猜测
		laser_est_yaw %= 2 * np.pi
		
		for i in range(len(LASER_ANGLES)):
			if i in laser_obstacle.laser_theory_length():
				continue
			else:
...
class Laser_obstacle:
	...
	def laser_theory_length(self):
		self.real_length = real.real_laser
		for i in range(len(LASER_ANGLES)):
			self.theory_length.append(realrobot.get_laser(i))##到时候Realrobot类可以作为理论长度计算而保留
		for i in range(len(LASER_ANGLES)):
...
laser_pos,laser_angle= estrobot.update_laser(laser_obstacle)
'卡尔曼滤波器'
############################################
class KalmanFilter:

	def __init__(self, dt=DT, process_noise_cov=np.eye(5), measurement_noise_cov=np.eye(3), state_cov=np.eye(5), initial_state_vector=[0, 0, 0, 0, 0],position=[0, 0],angle=0, state_dim=5):
		
		position, angle = estrobot.update_laser()
	...
	def predict(self):
		...
		if(globals.imu_data_flag==1):
			...
			self.state[4] += self.state[5] * self.dt +1/2*real.yaw_acc*(self.dt)**2# Update angle position
	...
	def update(self):
		...
		laser_position, laser_position_angle = estrobot.update_laser()
...
while True:
	...
	#添加卡尔曼滤波输出结果
	kalman_filter = KalmanFilter(dt=DT, 
							process_noise_cov=np.eye(5), 
							measurement_noise_cov=np.eye(3), 
							state_cov=np.eye(5), 
							initial_state_vector=np.matrix([0, 0, 0, 0, 0]))
	kalman = kalman_filter.update()
	kalman_pos=[kalman[0].item(),kalman[1].item()]
	kalman_angle=kalman[2].item()
	kalman_time = kalman_filter.get_time()
	kalman_time_diff = kalman_time - last_kalman_time
	last_kalman_time = kalman_time

	# 更新Kalmanfilter坐标和角度
	for i in range(2):
		estrobot.est_pos[i] = kalman_pos[i]
	estrobot.est_yaw = kalman_angle
	estrobot.est_vel = np.sqrt((kalman_pos[0] - last_kalman_pos[0])**2 + (kalman_pos[1] - last_kalman_pos[1])**2) / DT

	last_kalman_pos = kalman_pos
